...
class OPayController(http.Controller):
    ...
    @http.route('/payment/opay/acquirer_info', type='json', auth='public')
    def opay_acquirer_info(self, acquirer_id):
        ...
        return {
            ...
            'client_key': acquirer_sudo.opay_merchant_account,
        }

    @http.route('/payment/opay/payment_methods', type='json', auth='public')
    def opay_payment_methods(self, acquirer_id, amount=None, currency_id=None, partner_id=None):
        ...
        # OPay only supports a limited set of languages but, instead of looking for the closest
        # match in https://documentation.opayweb.com, we simply
        # provide the lang string as is (after adapting the format) and let OPay find the best fit.
        lang_code = (request.context.get('lang') or 'en-US').replace('_', '-')
        shopper_reference = partner_sudo and f'ODOO_PARTNER_{partner_sudo.id}'
        data = {
            'merchantAccount': acquirer_sudo.opay_merchant_account,
        ...
        response_content = acquirer_sudo._opay_make_request(
            url_field_name='opay_checkout_api_url',
    ...
    @http.route('/payment/opay/payments', type='json', auth='public')
    def opay_payments(
        ...
        :param int partner_id: The partner making the transaction, as a `r
        :param dict payment_method: The details of the payment method used for the transaction
        :param str access_token: The access token used to verify the provided values
        :param dict browser_info: The browser info to pass to OPay
        :return: The JSON-formatted content of the response
        :rtype: dict
        """
        ...
        # Make the payment request to OPay
        acquirer_sudo = request.env['payment.acquirer'].sudo().browse(acquirer_id).exists()
        ...
        data = {
            ...
            "reference": reference,
            "amount": {
                "total": converted_amount * 100,
                # "currency": 'EGP',
                "currency": request.env['res.currency'].browse(currency_id).name,
            },
            "returnUrl": urls.url_join(acquirer_sudo.get_base_url(),f'/shop/cart'),
            "callbackUrl": urls.url_join(acquirer_sudo.get_base_url(),f'/shop/cart'),
            "cancelUrl": urls.url_join(acquirer_sudo.get_base_url(),f'/shop/cart'),
            "expireAt": 300,
            "userInfo": {
                "userEmail": request.env['res.partner'].browse(partner_id).email,
                "userId": partner_id,
                "userMobile": request.env['res.partner'].browse(partner_id).mobile,
                "userName": request.env['res.partner'].browse(partner_id).name
            },
        ...
        tx_sudo.sudo()._handle_feedback_data(
            'opay', dict(response_content, merchantReference=reference),  # Match the transaction
        )
        return response_content

    @http.route('/payment/opay/payment_details', type='json', auth='public')
    def opay_payment_details(self, acquirer_id, reference, payment_details):
        ...
        :rtype: dict
        """
        # Make the payment details request to OPay
    ...
    @http.route('/payment/opay/return', type='http', auth='public', csrf=False, save_session=False)
    def opay_return_from_redirect(self, **data):
        """ Process the data returned by OPay after redirection.
        ...
        :param dict data: Feedback data. May include custom params sent to OPay in the request to
        ...
        # thought OPay is implemented as a direct payment provider, it will redirect the user out
        ...
        _logger.info("handling redirection from OPay with data:\n%s", pprint.pformat(data))
        self.opay_payment_details(
    ...
    @http.route('/payment/opay/notification', type='json', auth='public')
    def opay_notification(self):
        """ Process the data sent by OPay to the webhook based on the event code.

        See https://documentation.opayweb.com/ for the
        ...
        for notification_item in data['notificationItems']:
            ...
            try:
                ...
                PaymentTransaction.sudo()._handle_feedback_data('opay', notification_data)
